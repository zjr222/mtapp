<template>
  <dl class="s-item">
    ...
    <dd>
      <h3>
        <router-link  :to="{name: 'productDeatil',params:{title: meta.title}}">{{meta.title}}</router-link>
      </h3>
      <p>
        <el-rate
          v-model="meta.score"
      ...
      <p>
          <span class="s-item-tab">
              <span 
                v-for="(value,index) in meta.tab" :key="value"
              >
                {{ value + ((index - meta.tab.length - 1 > 0) ? '|' : '')}}
              </span>
          </span>
